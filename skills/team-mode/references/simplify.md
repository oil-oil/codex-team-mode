...
After edits, run the validation required by the task and project, and report which checks actually ran and their results. If any reviewed file changes after the Simplify review, dispatch one fresh `Reviewer` to inspect the resulting diff for correctness and omissions. If the reviewed files do not change, the original Reviewer may also serve as the final reviewer only when its brief explicitly included correctness and omission checks. Otherwise, dispatch a fresh Reviewer for the final check.
